...
import { page } from "$app/stores"
  // core components
  import { UserDropdown, NotificationDropdown } from "$lib/components";
      ...
      <ul class="md:flex-col md:min-w-full flex flex-col list-none">
        <li class="items-center">
          <a
            ...
            class="text-xs uppercase py-3 font-bold block {$page.url.pathname.indexOf('/admin/dashboard') !== -1 ? 'text-red-500 hover:text-red-600':'text-slate-700 hover:text-slate-500'}"
          >
            <i
              class="fas fa-tv mr-2 text-sm {$page.url.pathname.indexOf('/admin/dashboard') !== -1 ? 'opacity-75' : 'text-slate-300'}"
            ></i>
            Dashboard
          </a>
        </li>
